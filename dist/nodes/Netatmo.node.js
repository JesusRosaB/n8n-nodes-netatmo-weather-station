        ...
        for (let i = 0; i < items.length; i++) {
            try {
                if (operation === 'getStationData') {
                    ...
                    const getFavorites = this.getNodeParameter('getFavorites', i, false);
                    ...
                    const tokenResponse = await this.helpers.request({
                        ...
                        body: `grant_type=refresh_token&refresh_token=${encodeURIComponent(credentials.refreshToken)}&client_id=${credentials.clientId}&client_secret=${credentials.clientSecret}`,
                    });
